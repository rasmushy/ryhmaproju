...
(function (factory) {
  ...
  L.Control.Search = L.Control.extend({
    ...
    _defaultFormatData: function (json) {
      ...
      if (L.Util.isArray(propLoc)) for (i in json) jsonret[self._getPath(json[i], propName)] = L.latLng(json[i][propLoc[0]], json[i][propLoc[1]]);
      else for (i in json) jsonret[self._getPath(json[i], propName)] = L.latLng(self._getPath(json[i], propLoc));
    ...
    _handleArrowSelect: function (velocity) {
      ...
      for (var i = 0; i < searchTips.length; i++) L.DomUtil.removeClass(searchTips[i], "search-tip-select");
